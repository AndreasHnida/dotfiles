...
-- Closing
-- vim.keymap.set('n', 'q', '<cmd>:bd<cr>', { desc = 'Buffer delete' }) -- Handled by snacks.bufdelete
vim.keymap.set('n', 'Q', '<cmd>:qa!<cr>', { desc = 'Force Quit Vim' })

-- Diagnostics
vim.keymap.set('n', '<leader>q', vim.diagnostic.setloclist, { desc = 'Open diagnostic [Q]uickfix list' })

-- Terminalkeymaps (handled by snacks.terminal)
...
vim.keymap.set('v', '<C-c>', 'yy', { desc = 'Yank to clipboard' })
...
require('lazy').setup({
	...
	-- Replaced by snacks.nvim picker
	...
	},
	{
		...
		dependencies = {
			...
			-- Note: blink.cmp is configured separately below, this is just a dependency declaration
		},
		config = function()
			...
			local servers = {
				...
				-- TailwindCSS is handled by tailwind-tools.nvim plugin
			...
			local vue_plugin = {
				...
				location = "/home/user/.local/share/nvim/mason/packages/vue-language-server/node_modules/@vue/language-server",
				languages = { 'vue' },
			...
			vim.list_extend(ensure_installed, {
				...
				'tailwindcss-language-server', -- Managed by tailwind-tools.nvim but installed via Mason
			...
			require('mason-lspconfig').setup {
				...
				handlers = {
					function(server_name)
						-- Skip old TypeScript servers
						if server_name == 'ts_ls' or server_name == 'typescript-language-server' or server_name == 'typescript-tools' then
							return
						end

						-- Skip tailwindcss - handled by tailwind-tools.nvim
	...
	{ -- Autocompletion
		...
		opts = {
			-- Use 'super-tab' preset for easier completion
	...
	{ -- Shows context of the current function at the top of the screen
